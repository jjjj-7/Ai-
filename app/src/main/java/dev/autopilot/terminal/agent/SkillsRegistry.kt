...
object SkillsRegistry {
    ...
    val builtin: List<SkillDef> = listOf(
        ...
        SkillDef(
            "创造指令",
            "我想造一个环境里还没有的命令。先问我想要这个命令做什么、怎么调用 (参数设计), 然后把可执行脚本写进 ~/bin/<命令名> (shebang 用 /data/data/com.termux/files/usr 下的真实解释器路径, 脚本头部加 # 用途说明), chmod +x, 实际运行验证可用后, 把命令名与用法追加进 ~/bin/README.md, 最后告诉我用法示例; 也提醒我输入 mycmds 可随时查看全部自定义命令"
        )
    )
